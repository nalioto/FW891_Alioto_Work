...
new_points <- data.frame(northing = c(12000000, 11000000, 10500000),
                         easting = c(10000000, 13000000, 12500000))

#Convert now to a simple feature
new_points_sf <- st_as_sf(new_points,
                          coords = c("northing", "easting"),
                          crs = 3593)

# Adding our new points and changing their size and shape
plot5 = ggplot() +
  geom_sf(data = museums_SF2,
          mapping = aes(geometry = geometry),
          color = "red") +
  geom_sf(data = new_points_sf,
          mapping = aes(geometry = geometry),
          color = c("darkorchid3","orange", "yellow"),
          size = c(2,3,4),
          shape = c(22,23,8)) 
  coord_sf(crs = 26917);   # changing CRS
plot(plot5);
